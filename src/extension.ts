...
export function activate(context: vscode.ExtensionContext) {
  ...
  let disposable = vscode.commands.registerCommand('flowjam.showPreview', () => {
    // Add debug log
  ...
  // Handle editor switching - only update if the panel is already open
  vscode.window.onDidChangeActiveTextEditor(editor => {
    if (editor && (editor.document.languageId === 'yaml' || editor.document.fileName.endsWith('.yml'))) {
      const isVisible = currentPanel && currentPanel.visible;
      console.log('Editor changed to YAML file:', editor.document.fileName);
      currentEditor = editor;
      ...
      try {
        ...
        editor.edit((editBuilder: vscode.TextEditorEdit) => {
          ...
          if (success) {
            ...
            // Explicitly save changes using the document object for better reliability
            try {
